...
def printLst(List):
    try:
        assert Permissions is not None, 'We need initiate list'
        for x in List:
            print(x)
    except Exception as e:
        logger.error(e)

#This funciton permit write json list with the permissions of the app
#def writePermissions(path, lstP, lstD):
#    try:
#        assert os.path.isfile(path), '%s is not a valid path of json file' % path
#        assert lstP is not None, 'We need initiate %s list' % lstP
#        assert lstD is not None, 'We need initiate %s list' % lstD
#        f = open(path, 'w')
#        f.write('\t la aplicacion tiene estos permisos \n')
#        for x in lstResult:
#            f.write(x + '\n')
#        f.write('\t de los cuales se consideran peligros \n')
#        for x in lstDangerous:
#            f.write(x + '\n')
#    except Exception as e:
#        logger.error(e)
...
def writeJson(lstResult, lstDangerous):
    ...
    try:
        ...
        #assert os.path.isfile(path), '%s is not a valid path of json file' % path
        with open('permisos.json', 'w') as fp:
            fp.write(
