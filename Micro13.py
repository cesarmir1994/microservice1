...
def Service1():
    ...
    while bucle != 'exit':
        path = input()
        bucle = path
        condition = os.path.isfile(path)
        condition2 = fnmatch.fnmatch(path, '*.apk')
        if condition == False or condition2 == False:
            print(path + ' is a invalid rute to the apk file')
            logger.error(path + ' is a invalid rute to the apk file')
            continue
